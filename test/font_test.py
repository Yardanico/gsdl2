...
import gsdl2
from gsdl2 import font as pygame_font  # So font can be replaced with ftfont
from gsdl2.compat import xrange_, filesystem_errors
...
class FontTest(unittest.TestCase):
    ...
    def test_render_args(self):
        screen = gsdl2.display.set_mode((600, 400))
...
class FontTypeTest( unittest.TestCase ):
    ...
    def test_render(self):
        ...
        # __doc__ (as of 2008-08-02) for pygame_font.Font.render:
          ...
          # gsdl2 provides no way to directly draw text on an existing Surface:
    ...
    def test_font_file_not_found(self):
        # A per BUG reported by Bo Jangeborg on gsdl2-user mailing list,
        # http://www.mail-archive.com/gsdl2-users@seul.org/msg11675.html
    ...
    def test_load_from_file(self):
        ...
        font_path = os.path.join(os.path.split(gsdl2.__file__)[0],
    ...
    def test_load_from_file_unicode(self):
        base_dir = os.path.split(gsdl2.__file__)[0]
        sep = os.path.sep
        if sep == '\\':
            sep = '\\\\'
...
class VisualTests( unittest.TestCase ):
    ...
    def setUp(self):
        if self.screen is None:
            gsdl2.init()
            self.screen = gsdl2.display.set_mode((600, 200))
    ...
    def query(self,
        ...
        gsdl2.display.flip()
        while 1:
            for evt in gsdl2.event.get():
                if evt.type == gsdl2.KEYDOWN:
                    if evt.key == gsdl2.K_ESCAPE:
                        self.abort()
                        return False
                    if evt.key == gsdl2.K_y:
                        return True
                    if evt.key == gsdl2.K_n:
                        return False
                if evt.type == gsdl2.QUIT:
                    self.abort()
                    return False
