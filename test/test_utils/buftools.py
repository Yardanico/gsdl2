"""Module gsdl2.tests.test_utils.array

Export the Exporter and Importer classes.

Class Exporter has configurable shape and strides. Exporter objects
provide a convient target for unit tests on gsdl2 objects and functions that
...
This module contains its own unit tests. When gsdl2 is installed, these tests
can be run with the following command line statement:

python -m gsdl2.tests.test_utils.array

"""
import gsdl2
if not gsdl2.HAVE_NEWBUF:
    emsg = "This gsdl2 build does not support the new buffer protocol"
    raise ImportError(emsg)
import gsdl2.newbuffer
from gsdl2.newbuffer import (PyBUF_SIMPLE, PyBUF_FORMAT, PyBUF_ND,
...
class Exporter(gsdl2.newbuffer.BufferMixin):
...
class Importer(object):
    ...
    def __init__(self, obj, flags):
        self._view = gsdl2.newbuffer.Py_buffer()
