...
def run(*args, **kwds):
    """Run the gsdl2 unit test suite and return (total tests run, fails dict)
    ...
    recreates normal gsdl2 usage where gsdl2.init() and gsdl2.quit() are
    ...
    if not option_nosubprocess:
        if is_pygame_pkg:
            from gsdl2.tests.test_utils.async_sub import proc_in_time_or_kill
        else:
