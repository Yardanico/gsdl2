...
import gsdl2
from gsdl2.compat import long_
import math
...
class ColorTypeTest (unittest.TestCase):
    ...
    def test_ignore_whitespace(self):
        self.assertEquals(gsdl2.color.Color('red'), gsdl2.color.Color(' r e d '))
    ...
    def test_case_insensitivity_of_string_args(self):
        self.assertEquals(gsdl2.color.Color('red'), gsdl2.color.Color('Red'))

    def test_color (self):
        c = gsdl2.Color (10, 20, 30, 40)
        self.assertEquals (c.r, 10)
        self.assertEquals (c.g, 20)
        self.assertEquals (c.b, 30)
        self.assertEquals (c.a, 40)

        c = gsdl2.Color ("indianred3")
        self.assertEquals (c.r, 205)
        self.assertEquals (c.g, 85)
        self.assertEquals (c.b, 85)
        self.assertEquals (c.a, 255)

        c = gsdl2.Color (0xAABBCCDD)
        self.assertEquals (c.r, 0xAA)
        self.assertEquals (c.g, 0xBB)
        self.assertEquals (c.b, 0xCC)
        self.assertEquals (c.a, 0xDD)

        self.assertRaises (ValueError, gsdl2.Color, 257, 10, 105, 44)
        self.assertRaises (ValueError, gsdl2.Color, 10, 257, 105, 44)
        self.assertRaises (ValueError, gsdl2.Color, 10, 105, 257, 44)
        self.assertRaises (ValueError, gsdl2.Color, 10, 105, 44, 257)
    ...
    def test_webstyle(self):
        ...
        self.assertRaises (ValueError, gsdl2.Color, "#cc00qq")
        self.assertRaises (ValueError, gsdl2.Color, "0xcc00qq")
        self.assertRaises (ValueError, gsdl2.Color, "09abcdef")
        self.assertRaises (ValueError, gsdl2.Color, "09abcde")
        self.assertRaises (ValueError, gsdl2.Color, "quarky")

    def test_int (self):
        # This is a long
        c = gsdl2.Color (long_(0xCC00CC00))
        self.assertEquals (c.r, 204)
        self.assertEquals (c.g, 0)
        self.assertEquals (c.b, 204)
        self.assertEquals (c.a, 0)
...
################################################################################
    ...
    def test_correct_gamma__verified_against_python_implementation(self):
        ...
        for i, c in enumerate(rgba_combos_Color_generator()):
            ...
            corrected = gsdl2.Color(*[gamma_correct(x, gamma)
                                                 for x in tuple(c)])
    ...
    def test_pickle(self):
        import pickle
        c1 = gsdl2.Color(1,2,3,4)
        #c2 = gsdl2.Color(255,254,253,252)
...
# only available if ctypes module is also available
    ...
    def test_arraystruct(self):
        import gsdl2.tests.test_utils.arrinter as ai
        import ctypes as ct

        c_byte_p = ct.POINTER(ct.c_byte)
        c = gsdl2.Color(5, 7, 13, 23)
