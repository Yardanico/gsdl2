    ...
    def draw_walls(self, cell, color):
        ...
        if (wal & 0b0100):
            self.screen.fill(color, ((loc[0] * self.cell_size[0] + self.cell_size[0] + self.buffer[0],
                                      loc[1] * self.cell_size[1] + self.wall_size[1] + self.buffer[1]),
                                     (self.wall_size[0],
                                      self.core_size[1])))
...
class AMazeing(_CellDraw):
    ...
    def maze_bar(self):
        ...
        self.screen.blit(arialsmall.render(str(self.core_size[0]).zfill(2), 1, MYCOLORS["halls"]),
                         (763, MAZESIZE[1] + 41))
        self.screen.blit(arialsmall.render(str(self.wall_size[0]).zfill(2), 1, MYCOLORS["halls"]),
                         (671, MAZESIZE[1] + 41))
        if self.state == "START":
            self.screen.blit(arial.render("Please place your generation points.", 1, (0, 0, 0)), (270, MAZESIZE[1] + 5))
        elif self.state == "READY":
            self.screen.blit(arial.render("Press 'space' or click start to begin generation", 1, (0, 0, 0)),
                             (225, MAZESIZE[1] + 5))
            ...
            self.screen.blit(arial.render("Press 'space' or click start to solve your maze.", 1, (0, 0, 0)),
                             (225, MAZESIZE[1] + 5))
        if self.gentime:
            ...
            if self.animate:
                for Gen in self.genpoints:
                    ...
                    self.screen.fill((0, 0, 0), ((Gen.gencell[0][0] * self.cell_size[0] + self.buffer[0],
                                                  Gen.gencell[0][1] * self.cell_size[1] + self.buffer[1]),
                                                 self.cell_size))
                    if not self.difcol:
...
class Solver(_CellDraw):
    ...
    def get_path(self, cell):
        ...
        if cell in self.came_from:
            ...
            if direction in [(0, 1), (-1, 0)]:
                self.solwalls[self.currentcell] = self.solwalls.setdefault(self.currentcell, 0) | OPWALLS[
                    ADJCHECK[direction]]
            else:
