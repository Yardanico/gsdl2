...
from sdl import SCANCODE_RIGHT, SCANCODE_LEFT, SCANCODE_SPACE, SCANCODE_ESCAPE
...
def load_image(file, transparent):
    ...
    try:
        surface = gsdl2.image.load(file)
    except gsdl2.SDLError:
        raise SystemExit('Could not load image "%s" %s' %
                         (file, gsdl2.get_error()))
    if transparent:
...
def main():
    ...
    gsdl2.init()
    screen = gsdl2.display.set_mode(SCREENRECT.size, 0)
    ...
    background = gsdl2.Surface(SCREENRECT.size)
    ...
    while player.alive or explosions:
        ...
        # Gather Events
        gsdl2.event.pump()
        keystate = gsdl2.key.get_pressed()
        if keystate[SCANCODE_ESCAPE] or gsdl2.event.peek(QUIT):
            break
        ...
        # Move the player
        direction = keystate[SCANCODE_RIGHT] - keystate[SCANCODE_LEFT]
        player.move(direction)
        # Create new shots
        if not player.reloading and keystate[SCANCODE_SPACE] and len(shots) < MAX_SHOTS:
            shots.append(Shot(player))
        player.reloading = keystate[SCANCODE_SPACE]
